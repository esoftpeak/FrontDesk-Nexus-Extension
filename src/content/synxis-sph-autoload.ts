...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { display: flex; flex-direction: column; height: 100vh; font-family: sans-serif; background: #e0e0e0; overflow: hidden; }
    ...
    .pages { display: flex; align-items: center; gap: 10px; font-weight: 700; font-size: 1rem; letter-spacing: 0.08em; }
    .pages button {
      background: none; border: none; color: #fff; font-size: 1.3rem;
      cursor: pointer; line-height: 1; padding: 0 4px;
    }
    .pages button:hover { opacity: 0.7; }
    .cancel {
      display: flex; align-items: center; gap: 6px; background: none; border: none;
      color: #fff; font-weight: 700; font-size: 1rem; cursor: pointer; letter-spacing: 0.05em;
    }
    .cancel svg { width: 22px; height: 22px; }
    .cancel:hover { opacity: 0.7; }

    /* ── Sign banner ── */
    .sign-banner {
      background: #1976d2; color: #fff; text-align: center;
      padding: 7px 16px; font-size: 0.88rem; flex-shrink: 0;
    }

    /* ── PDF area ── */
    .pdf-wrap { flex: 1; overflow: hidden; position: relative; }
    embed { width: 100%; height: 100%; border: none; display: block; }

    /* ── Sign Here modal overlay ── */
    .sign-modal {
    ...
    .sign-box {
      ...
      box-shadow: 0 8px 32px rgba(0,0,0,0.35); display: flex; flex-direction: column; gap: 14px;
    }
    .sign-box h3 { font-size: 1rem; color: #1565c0; }
    canvas {
      border: 2px solid #1565c0; border-radius: 4px; cursor: crosshair;
      touch-action: none; background: #f8f9ff;
    }
    .sign-actions { display: flex; gap: 10px; justify-content: flex-end; }
    .sign-actions button {
      padding: 7px 20px; border-radius: 4px; border: none; font-weight: 600;
    ...
    .btn-cancel-sign { background: #f0f0f0; color: #333; }
    .btn-cancel-sign:hover { background: #e0e0e0; }
  </style>
</head>
<body>
  <div class="topbar">
    <div class="pages">
      ...
      PAGES &nbsp; <span id="pageLabel">1 / 1</span>
  ...
  <div class="sign-banner">
    To sign, click the <strong>Sign Here</strong> button below &nbsp;·&nbsp; Confirmation: <strong>${confirmation}</strong>
  </div>
  <div class="pdf-wrap">
  ...
  <div class="sign-modal" id="signModal">
    <div class="sign-box">
      ...
      <canvas id="sigCanvas" width="452" height="180"></canvas>
      <div class="sign-actions">
        <button class="btn-clear" onclick="clearSig()">Clear</button>
        <button class="btn-cancel-sign" onclick="closeSign()">Cancel</button>
        <button class="btn-save" onclick="saveSig()">Save Signature</button>
      </div>
    </div>
  </div>

  <!-- Floating Sign Here button -->
  <button onclick="document.getElementById('signModal').classList.add('open')"
    style="position:fixed;bottom:24px;right:24px;z-index:50;background:#1565c0;color:#fff;
           border:none;border-radius:28px;padding:12px 28px;font-size:1rem;font-weight:700;
  ...
  <script>
    ...
    let drawing = false;
    ctx.strokeStyle = '#1a237e'; ctx.lineWidth = 2; ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    ...
    function saveSig() {
      ...
      console.log('[FDN] Signature saved:', dataUrl.substring(0,60) + '...');
      closeSign();
    }

    // ── Page nav (PDF embed doesn't expose page API; buttons are UI placeholders) ──
    function prevPage() {}
    function nextPage() {}
  </script>
</body>
          ...
          window.open(htmlUrl, '_blank', 'popup,width=900,height=950,left=80,top=40')
        } catch (err) {
